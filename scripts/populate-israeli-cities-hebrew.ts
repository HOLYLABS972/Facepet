/**
 * Script to populate Israeli cities as audiences in Firestore (Hebrew names)
 * Run this script with: npx tsx scripts/populate-israeli-cities-hebrew.ts
 */
...
// 20 major Israeli cities in Hebrew
const israeliCitiesHebrew = [
  'ירושלים',
  'תל אביב',
  'חיפה',
  'ראשון לציון',
...
async function populateIsraeliCitiesHebrew() {
  console.log('🇮🇱 Starting to populate Israeli cities (Hebrew)...\n');
  ...
  try {
    ...
    for (const city of israeliCitiesHebrew) {
    ...
    console.log(`📍 Total cities: ${israeliCitiesHebrew.length}`);
    console.log('\n✨ Done!');
